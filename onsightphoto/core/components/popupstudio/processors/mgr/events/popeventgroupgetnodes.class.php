 ...
 */
class popEventGroupGetNodesProcessor extends modProcessor {
...
*/
		$parent = $this->getProperty('id', 'root');
		$parent = ($parent == 'root') ? 0 : $parent;		
		$nodes = $this->getEventGroupNodes($parent);
		
		return $this->toJSON($nodes);
    }

	public function getEventGroupNodes($parent) {
		
		//build the query to get the base groups.
	...
	protected function getSubEventGroups($groupId) {
        
        // get the collection of sub groups
        $q = $this->modx->newQuery('popEventGroup');
		$q->where(array(
            'parent' => $groupId
        ));
...
return 'popEventGroupGetNodesProcessor';
